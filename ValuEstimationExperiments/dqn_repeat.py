...
class TorchDQNLearning:
    """Deep Q-Network implementation with experience replay, target network, and action repetition"""
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        # Compute loss detaching target for TD update rule
...
class TorchDQNGDSTDMLearning(TorchDQNLearning):
    """
    Deep Q-Network implementation with GDS-TDM gradient update technique and action repetition
    ...
    3. Action repetition for temporal abstraction
    """
    def __init__(self, n_states, n_actions, args):
        super().__init__(n_states, n_actions, args)
        # Inherits all DQN components including networks, experience replay, and action repetition
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        self.policy_net.zero_grad()
        gd_loss = F.mse_loss(current_q_values, target_values)  # full autograd for GD (do not detach the target here)
        gd_loss.backward()
...
class TorchDQNGDLearning(TorchDQNLearning):
    """
    Deep Q-Network with Residual Gradient (GD) update and action repetition.
    Uses full autograd gradient of TD error.
    """
    def __init__(self, n_states, n_actions, args):
        super().__init__(n_states, n_actions, args)
    ...
    def update(self, state, action, reward, next_state, done):
        """GD update using full autograd gradient of TD error"""
        reward = np.clip(reward / 50.0, -2.0, 2.0)
        self.replay_buffer.add(state, action, reward, next_state, done)
