...
def train_sarsa(args: Namespace):
    ...
    for episode in range(args.episodes):
        ...
        if(args.method == "hybrid"):
            if(episode == 5000):
                update_fn = getattr(agent, f"update_gds_tdm")
        ...
        if (episode + 1) % args.validate_freq == 0:
            ...
            if mean_val > best_val_reward:
                ...
                torch.save(agent.weights, os.path.join(save_dir, 'best_weights.pt'))
            ...
            if args.use_wandb:
                wandb.log({
                    ...
                    'validation/best_reward': best_val_reward
                })
...
def main():
    ...
    save_path = os.path.join('results', args.save_name, latest_dir)

    best_model_path = os.path.join(save_path, 'best_weights.pt')
    if os.path.exists(best_model_path):
        agent.weights = torch.load(best_model_path)
    else:
        print("Best model not found, falling back to final model.")
        agent.weights = torch.load(os.path.join(save_path, 'final_weights.pt'))

    
    test_agent(agent, args.env, os.path.join('results', args.save_name, os.listdir(os.path.join('results', args.save_name))[-1]))
