...
from main import TorchSARSALearning, TorchSARSAGDSTDMLearning, ValidationEnv, validate_agent

class HybridSARSATrainer:
    def __init__(self, args):
        ...
        self.sarsa_agent = TorchSARSALearning(self.n_states, self.n_actions, args)
        self.gds_agent = None

        self.current_agent = self.sarsa_agent
        self.current_method = 'sarsa'
        self.early_stopping = EarlyStoppingMonitor(patience=args.early_stopping_patience)

        if args.use_wandb:
            wandb.init(
                project=args.wandb_project,
                name=args.wandb_name or f"hybrid_sarsa_{self.timestamp}",
                config=vars(args)
            )
    ...
    def switch_to_gds(self, episode):
        print(f"\n==== Switching from SARSA to SARSA-GDS-TDM at episode {episode} ====\n")
        self.gds_agent = TorchSARSAGDSTDMLearning(self.n_states, self.n_actions, self.args)
        self.gds_agent.weights = self.sarsa_agent.weights.clone()
        self.gds_agent.target_weights = self.sarsa_agent.target_weights.clone()
        self.gds_agent.velocity = self.sarsa_agent.velocity.clone()
        self.gds_agent.replay_buffer = deepcopy(self.sarsa_agent.replay_buffer)
        self.gds_agent.feature_extractor = deepcopy(self.sarsa_agent.feature_extractor)
        self.gds_agent.epsilon = self.sarsa_agent.epsilon
        self.gds_agent.lr = self.sarsa_agent.lr
        self.gds_agent.steps = self.sarsa_agent.steps

        self.current_agent = self.gds_agent
        self.current_method = 'sarsa-gds-tdm'
        self.training_history['switch_episode'] = episode

        if self.args.use_wandb:
            wandb.log({
                'switch/episode': episode,
                'switch/epsilon': self.sarsa_agent.epsilon,
                'switch/lr': self.sarsa_agent.lr
            })

    def train(self):
        for episode in range(self.args.episodes):
            if episode == self.args.switch_episode and self.current_method == 'sarsa':
                self.switch_to_gds(episode)

            self.current_agent.start_episode()
...
def parse_args():
    ...
    parser.add_argument('--min_episodes', type=int, default=10000)
    parser.add_argument('--save_name', type=str, default='hybrid_sarsa')
    parser.add_argument('--wandb_name', type=str, default='hybrid_sarsa')
...
def main():
    args = parse_args()
    trainer = HybridSARSATrainer(args)
    trainer.train()
