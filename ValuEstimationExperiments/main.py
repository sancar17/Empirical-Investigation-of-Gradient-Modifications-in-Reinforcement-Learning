...
class TorchLinearQLearning:
    """GPU-accelerated Linear Q-Learning with gradient clipping"""
    ...
    def repeat_action(self, env, state, action):
        ...
        For the non-repeating agent, this will be called with repeat_num=1.
        """
        # Get the repeat count (default to 1 if not set)
        repeat_count = getattr(self, 'repeat_num', 1)
        ...
        # Repeat the action repeat_count times or until done
...
class TorchGDSTDMLearning(TorchLinearQLearning):
    def update(self, state, action, reward, next_state, done):
        ...
        grad_gd = td_errors.view(-1, 1) * (current_features - self.gamma * next_features)
        ...
        del weights, current_q_values, next_q_values, q_sa
...
class TorchGDLearning(TorchLinearQLearning):
    """GPU-accelerated Residual Gradient (True Gradient Descent) Q-learning"""
    def update(self, state, action, reward, next_state, done):
        reward = np.clip(reward / 50.0, -2.0, 2.0)
        self.replay_buffer.add(state, action, reward, next_state, done)
...
def create_agent(args):
    ...
    if args.method == 'linear':
        ...
        return TorchGDLearning(n_states, n_actions, args)
